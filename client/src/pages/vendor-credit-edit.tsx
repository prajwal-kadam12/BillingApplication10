      ...
      if (item.id === itemId) {
        ...
        if (field === 'itemId') {
          ...
          if (value.includes('::')) {
            ...
            const billItem = billItems[parseInt(indexStr)];
            if (billItem) {
              ...
              updated.hsnSac = billItem.hsnSac || "";
            ...
            // Fallback for direct product selection
                            ...
                            >
                              ...
                              <SelectContent>
                                {productsLoading || billItemsLoading ? (
                                  ...
                                  billItems.map((bi: any, idx: number) => (
                  ...
                  <>
                    {gst.cgst > 0 && (
                      <div className="flex justify-between text-sm text-slate-600">
                        <span>CGST</span>
                        <span className="font-medium">{gst.cgst.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
                      </div>
                    )}
                    {gst.sgst > 0 && (
                      <div className="flex justify-between text-sm text-slate-600">
                        <span>SGST</span>
                        <span className="font-medium">{gst.sgst.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
                      </div>
                    )}
                    {gst.igst > 0 && (
                      <div className="flex justify-between text-sm text-slate-600">
                        <span>IGST</span>
                        <span className="font-medium">{gst.igst.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
                      </div>
                    )}
                  </>
                );
