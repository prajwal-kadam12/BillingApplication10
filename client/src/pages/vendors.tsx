                        ...
                        >
                            <div className="px-8 md:px-10 py-10 flex-1 flex flex-col">
                                    ...
                                    <div className="flex flex-col gap-4">
                                        ...
                                        <div>
                                            ...
                                            <div className="text-sm text-slate-600 mt-3 space-y-1">
                                                ...
                                                <div className="flex flex-wrap gap-x-4 gap-y-1 text-[11px] mt-1 text-slate-500">
                                                    {currentOrganization?.phone && <p>Phone: {currentOrganization.phone}</p>}
                                                    {currentOrganization?.email && <p>Email: {currentOrganization.email}</p>}
                                                </div>
                                ...
                                <div className="grid grid-cols-2 gap-12 mb-12">
                                    <div className="space-y-4">
                                        <div>
                                            <h3 className="text-xs font-bold text-slate-400 uppercase mb-2 tracking-widest">To:</h3>
                                            ...
                                            <div className="text-slate-600 leading-relaxed font-medium text-sm">
                                                ...
                                                <p>
                                                    {[vendor.billingAddress?.city, vendor.billingAddress?.state, vendor.billingAddress?.pinCode].filter(Boolean).join(', ')}
                                                </p>
                                            </div>
                                        </div>
                                        <div className="space-y-1 text-[11px] text-slate-500">
                                            {vendor.gstin && <p className="font-semibold text-slate-700">GSTIN: {vendor.gstin}</p>}
                                            {vendor.pan && <p>PAN: {vendor.pan}</p>}
                                            {vendor.email && <p>Email: {vendor.email}</p>}
                                            {(vendor.phone || vendor.mobile) && <p>Phone: {vendor.phone || vendor.mobile}</p>}
                                        </div>
                                    </div>
                                    <div>
                                        <div className="bg-slate-50 p-6 rounded-xl border border-slate-100">
                                            ...
                                            <div className="space-y-2">
                                                ...
                                                <div className="flex justify-between text-sm">
                                                    ...
                                                    <span className="font-semibold text-slate-900">{formatCurrency(transactions.bills.reduce((acc, b) => acc + b.amount, 0))}</span>
                                                </div>
                                                <div className="flex justify-between text-sm">
                                                    <span className="text-slate-600">Total Paid/Returned</span>
                                                    <span className="font-semibold text-slate-900">{formatCurrency(transactions.billPayments.reduce((acc, p) => acc + p.amount, 0) + (transactions.vendorCredits?.reduce((acc, vc) => acc + vc.amount, 0) || 0))}</span>
                                                </div>
                                ...
                                <table className="w-full text-sm mb-8">
                                    <thead>
                                        <tr className="border-b-2 border-slate-900 bg-slate-900 text-white">
                                            <th className="py-2 px-3 text-left font-bold uppercase tracking-wider text-[10px]">Date</th>
                                            <th className="py-2 px-3 text-left font-bold uppercase tracking-wider text-[10px]">Description</th>
                                            <th className="py-2 px-3 text-right font-bold uppercase tracking-wider text-[10px]">Purchases</th>
                                            <th className="py-2 px-3 text-right font-bold uppercase tracking-wider text-[10px]">Payments/Credits</th>
                                            <th className="py-2 px-3 text-right font-bold uppercase tracking-wider text-[10px]">Balance</th>
                                        </tr>
                                    </thead>
                                    <tbody className="divide-y divide-slate-100 text-slate-700">
                                        <tr className="bg-slate-50 border-b border-slate-200">
                                            <td className="py-3 px-3 italic font-medium">01/01/2026</td>
                                            <td className="py-3 px-3 font-bold text-slate-600 uppercase text-[10px]">Opening Balance</td>
                                            <td className="py-3 px-3 text-right font-medium text-slate-300">-</td>
                                            <td className="py-3 px-3 text-right font-medium text-slate-300">-</td>
                                            <td className="py-3 px-3 text-right font-bold text-slate-900">{formatCurrency(vendor.openingBalance || 0)}</td>
                                        </tr>
                                        {combinedTransactions.map((t) => (
                                            <tr key={`${t.tType}-${t.id}`} className="hover:bg-slate-50/50">
                                                <td className="py-3 px-3 font-medium text-slate-600">{formatDate(t.date)}</td>
                                                <td className="py-3 px-3">
                                                    <p className="font-bold text-slate-900 text-xs">{t.tType}: {t.number}</p>
                                                    {t.orderNumber && (
                                                        <p className="text-[10px] text-sidebar font-bold uppercase tracking-tighter">Ref: {t.orderNumber}</p>
                                                    )}
                                                </td>
                                                <td className="py-3 px-3 text-right font-medium text-slate-900">
                                                    {t.isCredit ? formatCurrency(t.displayAmount) : <span className="text-slate-300">-</span>}
                                                </td>
                                                <td className="py-3 px-3 text-right font-medium text-slate-900">
                                                    {!t.isCredit ? formatCurrency(t.displayAmount) : <span className="text-slate-300">-</span>}
                                                </td>
                                                <td className="py-3 px-3 text-right font-bold text-slate-900">{formatCurrency(t.runningBalance)}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>

                                <div className="border-t-2 border-slate-900 pt-8 mt-12 grid grid-cols-2 lg:grid-cols-2 gap-8 items-start mb-8">
                                    <div className="space-y-4">
                                        <div className="bg-slate-50 p-4 rounded-lg border border-slate-100 italic text-[11px] text-slate-600">
                                            <p className="font-bold uppercase tracking-wider mb-2 text-slate-900 not-italic">Notes:</p>
                                            <p>* Please quote statement period and vendor name in all correspondence.</p>
                                            <p>* This is an account confirmation statement generated from our digital records.</p>
                                        </div>
                                        <div className="pt-8 border-t border-slate-100 flex flex-col items-center justify-center w-48 text-center text-slate-400">
                                            <div className="h-0.5 w-full bg-slate-100 mb-2"></div>
                                            <p className="uppercase text-[9px] font-bold tracking-[0.2em]">Authorized Signature</p>
                                        </div>
                                    </div>
                                    <div className="flex justify-end">
                                        <div className="w-full max-w-[280px] space-y-4 bg-slate-900 p-6 rounded-xl text-white shadow-lg">
                                            <div className="flex justify-between items-center pb-3 border-b border-slate-700">
                                                <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Total Billed</span>
                                                <span className="font-bold text-sm tracking-tight">{formatCurrency(transactions.bills.reduce((acc, b) => acc + b.amount, 0))}</span>
                                            </div>
                                            <div className="flex justify-between items-center pb-3 border-b border-slate-700">
                                                <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Total Paid/Applied</span>
                                                <span className="font-bold text-sm tracking-tight">{formatCurrency(transactions.billPayments.reduce((acc, p) => acc + p.amount, 0) + (transactions.vendorCredits?.reduce((acc, vc) => acc + vc.amount, 0) || 0))}</span>
                                            </div>
                                            <div className="flex justify-between items-center pt-2">
                                                <span className="text-sm font-black uppercase tracking-widest text-white">Net Balance Due</span>
                                                <span className="text-xl font-black text-white">{formatCurrency(vendor.payables || 0)}</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>

                                <div className="mt-auto pt-10 text-[9px] text-slate-400 text-center uppercase tracking-[0.3em] font-medium border-t border-slate-100">
                                    Automated Statement | Computer Generated Report | Non-Solicitable
                                </div>
                            </div>
                        </div>
                    </div>
